...
本项目**默认集成 NVIDIA NIM (qwen/qwen3-next-80b-a3b-instruct)** 作为认知引擎。依托英伟达强大的推理算力，它能像人类分析师一样，对每一条资讯进行**评分、分类、提取摘要**，并过滤掉低价值的噪音。
...
* **🧠 多模型驱动**：
    * **默认 (推荐)**：集成 **NVIDIA NIM**，直接调用 **qwen/qwen3-next-80b-a3b-instruct** 模型，速度极快且极其稳定。
...
本项目默认配置为 **NVIDIA NIM (qwen/qwen3-next-80b-a3b-instruct)**，这目前是性价比最高、最稳定的方案。

1.  前往 **[NVIDIA Build 控制台](https://build.nvidia.com/)**。
...
只需配置 `NVIDIA_API_KEY` 即可，系统默认调用 `qwen/qwen3-next-80b-a3b-instruct`。
