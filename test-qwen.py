...
def list_models(client: OpenAI) -> None:
    # 输出到 stderr，避免污染 stdout（你要 stdout 纯 JSON 的话）
    try:
...
def run_with_retry(max_retries: int = 4) -> None:
    ...
    # 可选：python test-qwen.py --list-models
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if "404" in msg:
                print("[hint] 404 通常是模型名不对或账号没权限。先跑：python test-qwen.py --list-models 看你能用哪些模型。", file=sys.stderr)
            raise
